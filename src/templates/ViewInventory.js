...
class ViewInventory extends React.Component {
  ...
  fetchInventory = async () => {
    const result = await API.graphql({ query: listProducts })
  ...
  saveItem = async index => {
    ...
    const toUpdate = Object.assign({}, inventory[index])
  ...
  deleteItem = async index => {
    const itemToRemove = this.state.inventory[index]
    await API.graphql({
      query: deleteProduct,
      variables: {
        input: {
              ...
              <div className="flex items-center">
                ...
                <div className="flex flex-1 justify-end">
                  ...
                  <p className="m-0 pl-20 text-gray-900 tracking-tighter font-semibold">
                    {formatNum({ currency: DENOMINATION, number: item.price })}
                  </p>
                </div>
